...
# Тестовый скрипт для проверки передачи данных при потере команды на установление соединения
# Для теста делается запуск upd_processor с параметром --skip_connection=4
...
# Название теста
BTEST_TEST_NAME="Передача с потерей команды на соединение"
...
PROCESSORS="--skip_connection=4"

source ./fixture_processor.sh --skip_connection=4
...
function test() {
  ...
  if ! ${BIN_PATH}/file_generator --size=4000 --file=${SOURCE_FILE} ; then
