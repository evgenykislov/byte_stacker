...
# Тестовый скрипт для проверки передачи данных при задержках данных
...
# Название теста
BTEST_TEST_NAME="Передача с задержкой данных"
...
function test() {
  ...
  if ! ${BIN_PATH}/file_generator --size=100 --file=${SOURCE_FILE} ; then
