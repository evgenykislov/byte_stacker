...
# Тестовый скрипт для проверки передачи блоков данных в две внешние точки
# Тест работает по следующим пунктам:
# - запускает обе половинки транкового канала через фикстуру с поддержкой нескольких точек
...
# Название теста
BTEST_TEST_NAME="Передача 2 точки"
