...
void OutLink::RequestRead() {
  ...
  uint64_t r = read_volume_;
  uint64_t w = otherside_written_volume_;
  assert(
      w <= r);  // Обычно объём уже доставленных данным не больше отправленных
  if (w > r) {
    read_processing_ = false;
    CancelReadWrite();
    CheckReadyClose();
  ...
  uint64_t dw = r - w;  // Данных в доставке
