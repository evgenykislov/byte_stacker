 ...
 * Описание теста:
 * ===============
 * Тест проверяет, что при разрыве клиентского соединения с адресом address_from,
 * автоматически разрывается связанное с ним соединение на адресе address_to.
 * Это типичная ситуация для TCP прокси-серверов, мостов или туннелей.
 *
 * Сценарий теста:
 * ===============
 * 1. Тест устанавливает TCP соединение (как клиент) с адресом address_from
 * 2. Тест запускает TCP сервер на адресе address_to и ожидает входящего соединения
 ...
 * Примечания:
 * ===========
 * Тест предполагает наличие работающего TCP прокси/моста между address_from и address_to.
 * Если такой компонент отсутствует, тест не пройдет на этапе ожидания подключения к address_to.
 */
...
/**
 * @class TcpConnectionForwardingTest
 * @brief Класс теста для проверки переадресации TCP соединений
 */
 ...
 */
TEST_F(TcpConnectionForwardingTest, ConnectionClosePropagation) {
    ...
    ip::tcp::endpoint server_endpoint(
        ip::address::from_string(address_to_host),
        address_to_port
    );
        ...
        [&](const boost::system::error_code& error) {
            if (!error) {
                ...
                async_read(*accepted_socket, boost::asio::buffer(*buffer),
                    [&, buffer, accepted_socket](const boost::system::error_code& read_error, std::size_t) {
    ...
    ip::tcp::endpoint client_endpoint(
        ip::address::from_string(address_from_host),
        address_from_port
    );
    ...
    client_socket.async_connect(client_endpoint,
        [&](const boost::system::error_code& error) {
            if (error) {
                connect_promise.set_exception(
                    std::make_exception_ptr(
                        std::runtime_error("Connect to address_from failed: " + error.message())
                    )
                );
            } else {
    ...
    ASSERT_NO_THROW(connect_future.get())
        << "Connection to address_from failed";
